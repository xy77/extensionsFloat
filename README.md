...
朋友第一次安装可以使用你提供的 zip 包，不需要 GitHub、GitHub Desktop 或命令行下载源码。

1. 解压初始安装包到一个稳定目录，例如 `~/Applications/float-extension`。不要放在会被自动清理的位置。
...
最简单的方式是：朋友第一次 Load unpacked 后复制 Chrome 显示的 Extension ID，然后运行安装脚本。只要插件目录不移动，这个 ID 通常保持稳定。
